...
A hardening release: it fixes bugs that could crash the parser or silently give wrong results when reading and writing PDB and mmCIF files. The Rust API has no breaking changes. Three changes can affect existing code:

- **Python 3.10 or newer is required.** Python 3.9 reached end of life; on Python 3.9, `pip` keeps installing 0.7.0.
- **The PDB writer returns an error for values the PDB format cannot hold** (for example chain IDs longer than one character, which are common in large mmCIF structures). Python raises `ValueError`. 0.7.0 wrote a corrupted file instead. Write such structures as mmCIF.
- **Results change where 0.7.0 was wrong**, for example full multi-line titles, SSBOND symmetry operators, and title and resolution read from mmCIF files. Pipelines that compare against output from 0.7.0 may see differences.
...
- **Python bindings: upgrade PyO3 0.27 → 0.29 and rust-numpy 0.27 → 0.29**
  ...
  - pdbrust's bindings don't call the affected APIs; this upgrade itself doesn't change the Python API or results
...
- **PDB parser robustness**: files that previously crashed the parser or failed to load now parse.
  - No panics on short or non-ASCII lines, including REMARK lines with no text such as `REMARK   2` ([#16](https://github.com/HFooladi/pdbrust/pull/16), thanks to @kyclark). Columns are counted in characters, so a non-ASCII character no longer shifts the fields that follow it.
...
- **Python source distribution**: the sdist build failed because of a file clash between the library and the Python bindings, so releases shipped wheels only, and `pip install pdbrust` could not build from source on platforms without a pre-built wheel. The sdist now builds, CI installs from it to check it works, and it is published alongside the wheels.
...
### Added
- **RCSB search and download in Linux Python wheels** ([#8](https://github.com/HFooladi/pdbrust/issues/8)): Linux wheels were built without the `rcsb` feature because of OpenSSL cross-compilation problems. They now include it, as the macOS and Windows wheels do. This also fixes `from pdbrust import *`, which raised `AttributeError` on Linux.
...
- `Cargo.lock` is committed, so the minimum supported Rust version (1.85) is tested with compatible dependencies. CI also checks each feature on its own, builds and tests the Python bindings, and builds the sdist.
- New tests: property tests that the PDB parser never panics, PDB and mmCIF round trips, and Python tests that check files written by pdbrust with gemmi.
