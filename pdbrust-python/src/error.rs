...
pub fn convert_error(err: PdbError) -> PyErr {
    match err {
        // FileNotFoundError, PermissionError, ... (all subclasses of OSError/IOError)
        PdbError::IoError(e) => PyErr::from(e),
...
pub fn convert_download_error(err: pdbrust::rcsb::DownloadError) -> PyErr {
    ...
    match err {
        ...
        DownloadError::IoError(e) => PyErr::from(e),
    }
}
...
pub fn convert_selection_error(err: pdbrust::SelectionError) -> PyErr {
    ...
    match err {
        ...
        SelectionError::UnknownKeyword { keyword, position } => PyValueError::new_err(format!(
        ...
        SelectionError::UnclosedParenthesis { position } => {
            PyValueError::new_err(format!("Unclosed parenthesis at position {}", position))
        }
        SelectionError::EmptySelection => PyValueError::new_err("Selection string is empty"),
        SelectionError::SyntaxError { message, position } => PyValueError::new_err(format!(
