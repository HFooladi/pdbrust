...
fn parse_atom_record(line: &str) -> Result<Atom, PdbError> {
    ...
    let occupancy = if line.len() >= 60 { parse_float(&line[54..60])? } else { 1.0 };
    let temp_factor = if line.len() >= 66 { parse_float(&line[60..66])? } else { 0.0 };
    
    let element = if line.len() >= 78 { line[76..78].trim().to_string() } else { "".to_string() };
...
fn parse_conect_record(line: &str) -> Result<Conect, PdbError> {
    ...
    let atom2 = if line.len() >= 16 {
        ...
        return Err(PdbError::InvalidRecord("CONECT record missing second atom".to_string()));
    };
